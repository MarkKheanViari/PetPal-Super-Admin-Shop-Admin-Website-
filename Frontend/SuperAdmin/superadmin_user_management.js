...
async function addShopOwner(username, email, password) {
  try {
    const response = await fetch("http://192.168.1.13/backend/Frontend/SuperAdmin/add_shop_owner.php", {
      method: "POST",
...
async function saveUserChanges(payload) {
  try {
    const response = await fetch("http://192.168.1.13/backend/update_user.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
