...
async function fetchProducts(filter = "all", searchQuery = "") {
    ...
    try {
        const response = await fetch("http://192.168.1.12/backend/Frontend/SuperAdmin/fetch_products.php");
        const data = await response.json();
        allProducts = data.products || [];
        displayProducts(filter, searchQuery);
    } catch (error) {
        console.error("❌ Error fetching products:", error);
        productTable.innerHTML = "<tr><td colspan='7'>Error loading products</td></tr>";
    }
}
...
function displayProducts(filterType = "all", searchQuery = "") {
    ...
    if (filterType === "pending") {
        ...
        filteredProducts = filteredProducts.filter((product) => product.status === "Approved");
    } else if (filterType === "rejected") {
        filteredProducts = filteredProducts.filter((product) => product.status === "Rejected");
    }
        ...
        filteredProducts.forEach((product) => {
            const row = `
                <tr>
                    ...
                    <td><span class="status-${(product.status || "unknown").toLowerCase()}">${product.status || "Unknown"}</span></td>
                    <td>
                        <button class="approve-btn" data-id="${product.id}">✔</button>
                        <button class="reject-btn" data-id="${product.id}">✘</button>
                        <button class="more-btn" data-id="${product.id}">More</button>
                    </td>
                </tr>
            `;
...
function setupEventListeners() {
    ...
    document.querySelectorAll(".filter-btn").forEach((button) => {
        button.addEventListener("click", function () {
            ...
            displayProducts(filterType, document.getElementById("searchBar").value.trim().toLowerCase());
        });
    });
...
async function updateProductStatus(productId, status) {
    try {
        const response = await fetch("http://192.168.1.12/backend/Frontend/SuperAdmin/update_product_status.php", {
            method: "POST",
        ...
        if (result.success) {
            ...
            fetchProducts(currentFilter, document.getElementById("searchBar").value.trim().toLowerCase());
        } else {
            throw new Error(result.message || `Failed to ${status.toLowerCase()} product`);
        }
    } catch (error) {
