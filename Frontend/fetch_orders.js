      ...
      if (data.success) {
        ...
        if (data.order.items && data.order.items.length > 0) {
          data.order.items.forEach((item) => {
            const tr = document.createElement("tr");
            const total = (
              parseFloat(item.price) * parseInt(item.total_quantity)
            ).toFixed(2);
            tr.innerHTML = `
                            <td>${item.product_name || "Unknown Product"}</td>
                            <td>${item.total_quantity || 0}</td>
                            <td>PHP ${parseFloat(item.price || 0).toFixed(
                              2
                            )}</td>
                            <td>PHP ${total}</td>
                        `;
...
function downloadReceipt() {
  ...
  const sellerName =
    localStorage.getItem("shop_owner_username") || "PetPal Seller";
  ...
  doc.setFontSize(8);
  doc.text("Generated on: " + new Date().toLocaleString(), 105, footerY + 10, {
    align: "center",
  });
