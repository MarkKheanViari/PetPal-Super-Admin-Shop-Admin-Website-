...
async function fetchProducts() {
  try {
      const userId = localStorage.getItem('shop_owner_id');
      const response = await fetch(`http://192.168.43.55/backend/fetch_product.php?shop_owner_id=${userId}`);
...
async function fetchServices() {
  try {
      const userId = localStorage.getItem('shop_owner_id');
      const response = await fetch(`http://192.168.43.55/backend/fetch_services.php?shop_owner_id=${userId}`);
      if (!response.ok) {
...
const response = await fetch('http://192.168.43.55/backend/update_service_status.php', {
  method: 'POST',
  headers: {
...
document.getElementById('productForm').addEventListener('submit', async (e) => {
  ...
  try {
      const response = await fetch('http://192.168.43.55/backend/add_product.php', {
          method: 'POST',
          body: formData,
      });
