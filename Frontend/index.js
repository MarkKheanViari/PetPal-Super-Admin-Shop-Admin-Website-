document.addEventListener("DOMContentLoaded", function () {
  ...
  if (editProductForm) {
    editProductForm.addEventListener("submit", async (e) => {
      ...
      try {
        const response = await fetch("http://192.168.1.12/backend/update_product.php", {
          method: "POST",
          body: formData,
        });
  ...
  if (productForm) {
    productForm.addEventListener("submit", async (e) => {
      ...
      try {
        const response = await fetch("http://192.168.1.12/backend/add_product.php", {
          method: "POST",
          body: formData,
        });
...
function fetchProducts() {
  ...
  const url = `http://192.168.1.12/backend/fetch_product.php?shop_owner_id=${shopOwnerId}&category=${category}`;
...
async function deleteProduct(productId) {
  ...
  fetch("http://192.168.1.12/backend/delete_product.php", {
    method: "POST",
...
function filterProducts() {
  ...
  let url = `http://192.168.1.12/backend/fetch_product.php?shop_owner_id=${shopOwnerId}`;
  if (category !== "all") {
...
function showProductPreview(product) {
  ...
  if (!imagePath.startsWith("http")) {
      imagePath = `http://192.168.1.12/backend/uploads/${imagePath}`;
  }
