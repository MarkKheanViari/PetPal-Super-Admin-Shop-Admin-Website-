document.addEventListener("DOMContentLoaded", function () {
    ...
    if (editProductForm) {
      editProductForm.addEventListener('submit', async (e) => {
          ...
          try {
              const response = await fetch('http://192.168.168.203/backend/update_product.php', {
                  method: 'POST',
                  body: formData
              });
    ...
    if (productForm) {
      productForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('http://192.168.168.203/backend/add_product.php', {
                method: 'POST',
                body: formData,
            });
  ...
  function fetchProducts() {
    ...
    const url = `http://192.168.168.203/backend/fetch_product.php?shop_owner_id=${shopOwnerId}&category=${category}`;
  ...
  function displayProducts(products) {
    ...
    products.forEach(product => {
        ...
        if (!imagePath.startsWith("http") && !imagePath.startsWith("/")) {
            imagePath = `http://192.168.168.203/backend/uploads/${imagePath}`;  // Adjust path
        }
  ...
  function viewOrderDetails(orderId) {
    fetch(`http://192.168.168.203/backend/fetch_order_details.php?order_id=${orderId}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
  ...
  function deleteProduct(productId) {
    ...
    fetch('http://192.168.168.203/backend/delete_product.php', {  
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            fetch('http://192.168.168.203/backend/fetch_product.php?refresh=true')
  ...
  function showProductPreview(product) {
      let imagePath = product.image;
      if (!imagePath.startsWith("http") && !imagePath.startsWith("/")) {
        imagePath = `http://192.168.168.203/backend/uploads/${imagePath}`;
      }
