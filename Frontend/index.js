document.addEventListener("DOMContentLoaded", function () {
    ...
    editProductForm.addEventListener("submit", async (e) => {
      ...
      try {
        const response = await fetch(
          "http://192.168.168.55/backend/update_product.php",
          {
            method: "POST",
            body: formData,
          }
  ...
  if (productForm) {
    productForm.addEventListener("submit", async (e) => {
      ...
      try {
        const response = await fetch(
          "http://192.168.168.55/backend/add_product.php",
          {
            method: "POST",
            body: formData,
          }
...
function viewOrderDetails(orderId) {
  fetch(
    `http://192.168.168.55/backend/fetch_order_details.php?order_id=${orderId}`
  )
    .then((response) => response.json())
    .then((data) => {
      if (data.success) {
  ...
  fetch("http://192.168.168.55/backend/delete_product.php", {
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        fetch(
          "http://192.168.168.55/backend/fetch_product.php?refresh=true"
