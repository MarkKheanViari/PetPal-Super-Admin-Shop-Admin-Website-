...
function fetchAllAppointments() {
  fetch("http://192.168.1.13/backend/fetch_all_appointments.php")
...
function fetchProducts() {
  fetch("http://192.168.1.13/backend/fetch_product.php")
    .then((response) => response.json())
    .then((data) => {
      if (data.success) {
...
function fetchAppointmentNotifications() {
  const groomingPromise = fetch(
    "http://192.168.1.13/backend/fetch_grooming_appointments.php"
  )
    .then((response) => response.json())
    .then((data) => {
  ...
  const vetPromise = fetch(
    "http://192.168.1.13/backend/fetch_veterinary_appointments.php"
  )
    .then((response) => response.json())
    .then((data) => {
      if (data.success && Array.isArray(data.appointments)) {
